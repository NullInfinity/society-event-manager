...
class MemberDatabaseTestCase(unittest.TestCase):
    ...
    def test_getMember_barcodeAndNames_present_unique_noUpdateTimestamp(self, mocksql_connect):
        mdb = MemberDatabase('test.db')
        mdb.optionalCommit = MagicMock()
        mocksql_connect().cursor().fetchall.return_value = [(self.firstName, self.lastName)]

        self.assertEqual((self.firstName, self.lastName), mdb.getMember(self.memberId, firstName=self.firstName, lastName=self.lastName, updateTimestamp=False, autoFix=False))
        mocksql_connect().cursor().execute.assert_called_once_with('SELECT firstName,lastName FROM users WHERE barcode=?', (self.memberId,))
        self.assertEqual(1, mocksql_connect().cursor().execute.call_count)
        self.assertEqual(1, mocksql_connect().cursor().fetchall.call_count)
        self.assertTrue(mdb.optionalCommit.called)
    ...
    def test_getMember_namesOnly_present_unique_noUpdateTimestamp_autoFix(self, mocksql_connect):
        mdb = MemberDatabase('test.db')
        mdb.optionalCommit = MagicMock()
        mocksql_connect().cursor().fetchall.return_value = (self.firstName, self.lastName)

        self.assertEqual((self.firstName, self.lastName), mdb.getMember(None, firstName=self.firstName, lastName=self.lastName, updateTimestamp=False, autoFix=True))
        mocksql_connect().cursor().execute.assert_called_once_with('SELECT firstName,lastName FROM users WHERE firstName=? AND lastName=?', (self.firstName, self.lastName,))
        self.assertEqual(1, mocksql_connect().cursor().execute.call_count)
        self.assertEqual(1, mocksql_connect().cursor().fetchall.call_count)
